...
class SignUp extends Component {
	...
	onSubmit = (e) => {
		...
		if (this.state.fName == '' || this.state.lName == '' || this.state.email == '' || this.state.phone == '' || this.state.gender == '' || this.state.subj1 == undefined || this.state.subj2 == undefined || this.state.subj3 == undefined || this.state.subj4 == undefined) {
			err = 1;
		}
		...
		if (err == 1) {
			...
			console.log(this.state.fName.trim(), this.state.lName.trim(), this.state.oName.trim(), this.state.phone.trim(), this.state.email.trim(), this.state.tca, this.state.tcc, this.state.gender.trim(), this.state.subj1, parseInt(this.state.subj2), parseInt(this.state.subj3), parseInt(this.state.subj4));
			// const uniqueId = DeviceInfo.getUniqueID();
			axios.post('https://386cf5fa830b.ngrok.io/api/register', {
					...
					db.addUser(uData).then((res) => {
						if (res.rowsAffected == 1) {
							...
							for (let i = 0; i < 4; i++) {
								...
								axios.get('https://386cf5fa830b.ngrok.io/api/getTopicsInSubjects/' + subjId, {})
									.then(res => {
										...
										if (topicLen > 0) {
											for (k = 0; k < topicLen; k++) {
												...
												for (l = 0; l < topicObjLen; l++) {
													...
													axios.get('https://386cf5fa830b.ngrok.io/api/getQuestionsInObjective/' + obj.id, {})
														.then(res => {
	...
	render() {
		...
		if (this.state.nameVisible == true && this.state.phoneAddVisible == false && this.state.subjVisible == false) {
		...
		else if (this.state.nameVisible == false && this.state.phoneAddVisible == true && this.state.subjVisible == false) {
			viewFormSection = <View>
				...
				<View style={styles.row}>
					...
					<View style={styles.button}>
						<Button buttonStyle={{ backgroundColor: '#F93106' }} title="Next" onPress={() => { this.toSubj(); }} />
					</View>
		...
		else if (this.state.nameVisible == false && this.state.phoneAddVisible == false && this.state.subjVisible == true) {
			viewFormSection = <View>
				<Item picker>
					<Picker
						mode="dropdown"
